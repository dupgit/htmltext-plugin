...
A Claws Mail plugin that renders `text/html` MIME parts as plain text.

## Architecture

```
claws-mail
  └── htmltext_viewer.so   (this directory – C plugin)
        │  registers a MimeViewer for text/html
        │  extracts MIME part to temp file
        │  pipes raw HTML to stdin of:
        └── htmltext-render   (../render – Rust binary)
...
Load the plugin in Claws Mail: *Configuration → Plugins → Load plugin…*
