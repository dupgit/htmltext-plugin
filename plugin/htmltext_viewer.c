 ...
 *
 * Build note:
 *   We compile with -UHAVE_CONFIG_H so that Claws headers that guard
 *   `#include "config.h"` with `#ifdef HAVE_CONFIG_H` skip that include.
 *   config.h and claws-features.h are generated by Claws' autotools build
 *   and are NOT installed with the development headers package.
 *   The only symbol we actually need from them is VERSION, which we define
 *   below, and MAKE_NUMERIC_VERSION, which is declared in plugin.h itself.
 */

/* Must come before any Claws header to prevent HAVE_CONFIG_H being set
